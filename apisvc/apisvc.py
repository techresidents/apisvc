...
from handler import ApiServiceHandler, ApiMongrel2Handler


class ApiService(GDefaultService):
    """API service."""
    def __init__(self):
        handler = ApiServiceHandler(self)
        ...
        server = GThriftServer(
                ...
                handler=handler,
                processor=TApiService.Processor(handler),
                address=settings.THRIFT_SERVER_ADDRESS)

        mongrel2_handler = ApiMongrel2Handler(handler)
